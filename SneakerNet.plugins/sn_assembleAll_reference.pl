...
our $VERSION = "2.0";
our $CITATION= "Reference assembly plugin by Lee Katz.";
...
sub main{
  ...
  my @exe = (
    ...
    "cat", "wget", "samtools", "bcftools", "trimmomatic",
    "seqtk", "bgzip", "tabix", "bowtie2", "bowtie2-build",
  );
...
sub assembleAll{
  ...
  while(my($sample,$info)=each(%$sampleInfo)){
    ...
    if(! -e $asmMetricsFile){
      ...
      command("run_assembly_metrics.pl -m 1 --allMetrics --numcpus $$settings{numcpus} $outassembly >> $asmMetricsFile.tmp");
...
sub assembleSample{
  ...
  if($$sampleInfo{taxonRules}{primers_bed}){
    ...
    logmsg "Primers not found (usually defined in taxonProperties.conf) and so not trimming.";
  }
...
sub annotateFastaWithBioPerl{
  ...
  #$refSeqCdsCounter ||= ~0;
  my $divideBy = $refSeqCdsCounter || ~0;
  my $percentCds = sprintf("%0.2f", $altSeqCdsCounter/$divideBy);
